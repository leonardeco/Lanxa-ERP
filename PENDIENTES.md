...
Backlog vivo del proyecto. Actualizado: **14 de julio de 2026** (31ª revisión — 🚀 **Run 1 — Fundación Postgres** (PR [#26](https://github.com/leonardeco/superozono-erp/pull/26), primer paso de la migración a SaaS multi-tenant del ADR 0001): la suite de tests y el CI pasan de SQLite a **PostgreSQL** (motor de prod/nube, requisito de RLS); CI valida migraciones + `alembic check` (**blinda el drift #10**); SQLite sigue soportado para el LAN. 285 tests verdes en PG. Sin lógica de tenant aún (Runs 2–5). — 30ª revisión — ✅ **seeder de datos demo** mergeado (PR [#23](https://github.com/leonardeco/superozono-erp/pull/23), squash `7bd1fbd`): CLI `backend/seeds/seed_demo.py`, BD demo dedicada `superozono_demo.db` con guard anti-producción, 8 tests. Con esto la sección "🔵 codeable sin bloqueo" queda **vacía** — lo que resta es de negocio/contadora u operativo/servidor. — 29ª revisión — 🧹 corrección de drift: el módulo lote+vencimiento (PR [#16](https://github.com/leonardeco/superozono-erp/pull/16)) ya está mergeado y movido a completados (DOCUMENTACION.md §13 #51), se retiró de la tabla de features pendientes; ✅ Dependabot: fusionados #13 (pip menores) y #14 (npm menores), cerrado #15 (typescript 6→7, bloqueado por peer dep de `typescript-eslint <6.1.0`). — 28ª rev: ✅ **nuevo rol Contador** (PR [#18](https://github.com/leonardeco/superozono-erp/pull/18), área contable; ventas/compras solo consulta); ✅ **#13 servicios de dominio de ventas** (PR [#17](https://github.com/leonardeco/superozono-erp/pull/17)); ✅ módulo lote+vencimiento completo; 🔧 drift de Alembic de la BD local reconciliado (ver #10)). **Este archivo es la fuente única de pendientes.**
Estado general: 285 tests API + 66 componentes + 5 E2E (local y CI), CI verde, 0 CVEs. Versión: **v0.3.0**.
**Run 2+3 (2026-07-15):** `tenants` + `tenant_id` + JWT/contextvar + secuencias por tenant; **RLS Postgres** (`f7a8b9c0d1e2` + `apply_rls_tenant`). SQLite LAN = no-op RLS. Run 4 = filtros explícitos en listados + tests HTTP.
