...
- **Run 2 — Tenancy foundation (2026-07-15):** modelo `Tenant` + mixin `TenantScoped` (`tenant_id`) en tablas de negocio; migración `e6f7a8b9c0d1` (seed empresa #1 `superozono`); JWT con claim `tenant_id`; `get_current_user` fija contextvar; `document_sequences` PK `(tenant_id, prefix)`. **Aún sin RLS** (Run 3) ni filtros automáticos en todos los listados (Run 4). Plan: `docs/hydraia/plans/2026-07-15-run2-tenancy-foundation.md`.
