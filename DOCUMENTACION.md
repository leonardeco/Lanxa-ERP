...
- **Run 3 — RLS PostgreSQL (2026-07-15):** migración `f7a8b9c0d1e2` (ENABLE+FORCE RLS + policy `tenant_isolation` solo en PG); `apply_rls_tenant` / `set_config('app.tenant_id')` en `get_db` y post-login; tests de aislamiento en `test_tenancy.py`. SQLite LAN: no-op. Plan: `docs/hydraia/plans/2026-07-15-run3-rls-postgres.md`. **Run 4 pendiente:** filtros explícitos en listados + tests HTTP multi-módulo.
